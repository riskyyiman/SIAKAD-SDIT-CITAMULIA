...
import { getSiswa } from '@/actions/siswa'; // Pastikan path benar

export default function KelolaSiswaPage() {
  const [students, setStudents] = useState<any[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [showAll, setShowAll] = useState(false); // State untuk kontrol limit
  ...
  const filteredStudents = students.filter((s) => s.nama.toLowerCase().includes(searchTerm.toLowerCase()) || s.nis.includes(searchTerm));

  // Logika pemotongan data (Tampilkan 5 atau semua)
    ...
    <div className="p-8 bg-[#FFF5F5] min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manajemen Data Peserta Didik SDIT Cita Mulia </p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-50">
        <div className="flex justify-between items-center mb-6 gap-4">
          ...
          {/* Tombol Tambah Siswa [cite: 41, 114] */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="text-sm">
              {displayedStudents.map((siswa) => (
                <tr key={siswa.nis} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-all">
                  <td className="py-4 px-4 flex items-center gap-3">
                    <div className="w-10 h-10 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center font-bold text-[10px]">{siswa.nama.substring(0, 2).toUpperCase()}</div>
                    <div>
        ...
        {filteredStudents.length > 5 && (
          <div className="mt-6 flex justify-center">
            <button onClick={() => setShowAll(!showAll)} className="flex items-center gap-2 text-[#2D7A32] font-bold text-sm hover:underline">
              {showAll ? (
                <>
                  {' '}
                ...
                <>
                  {' '}
                  <ChevronDown size={18} /> Tampilkan Semua ({filteredStudents.length} Siswa){' '}
                </>
              )}
            </button>
          </div>
